...
The **Redefine Keys** section of the Preferences allows you to change the mappings for the various [Keyboard Shortcuts](../../IDE_Navigation/Keyboard_Shortcuts.md) provided in the [GameMaker IDE](../../The_IDE/The_IDE.md).
...
This window will show the location and name of the command that is already using the key combination that you are trying to use. In case the key combination is used by multiple commands, it will show all of them in a list.  
...
Click **Export as file** to bring up a file dialog that prompts you for the location to save the file to. This button can only be used when at least one mapping has been changed.
...
Click **Import file** to load a previously exported remaps file and use them in the current GameMaker build on the current machine.
  ...
  Any existing remaps will be lost if you import remaps from a file.
