# How to Disable An Extension For A Target?
...
Select the configuration for one target platform. Let's assume you selected the **GDK** configuration.

Open an extension that should not be used for GDK. For this example, open the Steamworks extension.

Disable all of the targets under **Copies To**. This will ensure that the Steamworks extension is not executed when using the GDK configuration.
