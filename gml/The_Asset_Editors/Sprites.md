...
- **Rectangle With Rotation**: This is a simple rectangular collision mask which will rotate along with the [image\_angle](../GameMaker_Language/GML_Reference/Asset_Management/Sprites/Sprite_Instance_Variables/image_angle.md) value of the instance with the sprite assigned. This is slower to resolve than the regular rectangular collision mask, but faster than a precise collision mask.
...
The **Nine Slice** section of the Sprite Editor allows you to enable Nine Slicing on a sprite, and preview it in action inside the **Nine Slice Preview** window on the right. Note that this is not supported for [Non\-Bitmap Sprites](../Settings/Texture_Information/Non-Bitmap_Sprites.md).
...
- **Activate Nine Slice:** This option controls whether Nine Slice scaling is used when the sprite is scaled. When enabled, Nine Slice scaling will be used for the sprite (and any object instances using that sprite) placed in a Sequence or a Room Layer, and scaled sprites drawn using [draw\_sprite\_ext](../GameMaker_Language/GML_Reference/Drawing/Sprites_And_Tiles/draw_sprite_ext.md) and other similar functions.  
  ...
  Nine Slicing will not work with functions that draw only a part of the sprite or distort it, such as [draw\_sprite\_part](../GameMaker_Language/GML_Reference/Drawing/Sprites_And_Tiles/draw_sprite_part.md) or [draw\_sprite\_pos](../GameMaker_Language/GML_Reference/Drawing/Sprites_And_Tiles/draw_sprite_pos.md).
...
- **Selected Slice:** This option can be used to select a slice, choosing one from the edge slices (left, top, right, bottom) and the centre one. A slice can also be selected by  left clicking on it in the **Sprite Preview** window, and clicking on it again will de\-select it.
...
Once added, the message will be shown in the Frame View, and you can click the right mouse button  on it to open a menu where you can delete the message or edit it. See [Broadcast Messages \& Moments](Sequence_Properties/Broadcast_Messages.md) for more info.
...
- 
- **View Vector Wireframe** : For SVG files, this will switch between previewing the sprite normally, displaying the wireframe or displaying both.
...
The **Playback controls** are for playing back in the preview window the animation of the frames, with the play button  starting/stopping the animation and the Loop button  used to set whether the sprite loops  (goes back to frame 1 when the last frame is reached) or ping\-pongs  (goes backwards through the frames when the last frame is reached).
...
An additional part of the Frame Controls is the **current frame** display and input a bit further over on the left. The box shows the total number of frames of the sprite animation as well as the current frame being shown. You can change the current frame at any time by clicking on the input box and typing in the frame number you want to view.
  ...
  Frames are shown numbered from 1 to *maxframe number*, however in your game code, frames will be numbered from 0 to *maxframe number \- 1*.

[Frames](#)

This part of the sprite editor shows each of the different **frames** that make up a single sprite set along a sequence timeline. You can click  on any frame and drag it left or right to change the order in which they will be animated, and you can also double click  on any frame to launch [The Image Editor](Image_Editor.md) and edit them. You can cut, copy and paste frames using the standard keyboard shortcuts or by using the right\-mouse button  menu options, and you can also select multiple frames to operate on using  /  \+ .

You can also drag and drop image files into this view to add them as additional frames for the sprites. Note that adding a vector frame onto a bitmap sprite will convert the vector into bitmap, and adding a bitmap frame onto a vector sprite is not allowed.
...
- **Delete Selected Frames** \- Delete the currently selected frame or frames.
  ...
  "**Delete Selected Frames**" is the only option available for [vector sprites](../Settings/Texture_Information/Non-Bitmap_Sprites.md).
...
[The Inspector](../IDE_Tools/The_Inspector.md) displays an additional option when a Sprite is selected:

- **Import Strip**: This option lets you pick an image to import, and opens the "Convert to Frames" window from [The Image Editor](Image_Editor.md) (mentioned under the "Image" menu). It lets you convert a single image (i.e. a sprite sheet) into a series of frames that form an animation. You can import bitmap and SVG images as strip images.
