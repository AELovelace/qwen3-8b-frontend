...
In the Room Editor, you can add a new Filter/Effect layer by clicking on the  button, which will add a new layer and open up its properties below:
...
Adding an FX layer will apply the selected effect to all other layers that are *below* the FX layer, as demonstrated in this visual:
...
To create filters/effects that are applied to only one layer, see: [Single\-Layer FX](#h).

NOTE Modifying the [depth of a layer](../../GameMaker_Language/GML_Reference/Asset_Management/Rooms/General_Layer_Functions/layer_depth.md) or [an instance](../../GameMaker_Language/GML_Reference/Asset_Management/Instances/Instance_Variables/depth.md) at runtime may change which FX layers are applied on it, by either making its depth higher than an FX layer's depth (essentially going *under* the FX layer and being affected) or making its depth lower than an FX layer's depth (going *above* the FX layer and not being affected).
...
You can also click on the  icon to enable/disable the FX for the selected layer.

  There are some filters/effects that can **only be applied to single layers** (such as "Blend") and will only appear for non\-FX layers.
