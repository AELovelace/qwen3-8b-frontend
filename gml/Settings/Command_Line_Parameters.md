...
When creating games it is important that you play test the executable file to make sure that the final compile is correct and has no unforeseen errors or differences. However, creating an executable means that, since you have no access to the debug console nor to the compiler window, it can be difficult to pin down and debug any problems that you may find. However, there is a fix for this, and that is to use **command line parameters** to set certain properties when the executable version of the game is run, in turn permitting you to create output log files and test other special situations.
  ...
  This is available on the standard **Windows** target module **only**.

To get this working you must first create a Windows executable of your game and have it installed on your test machine. Once you have that done, you need to create a *Shortcut* to the game on the desktop. This is where we will add in the parameters to be passed to the \*.exe (to create a shortcut, right click  the game executable and then select *Send to* \> *Desktop (create shortcut)*) .
...
\-output \: Sends console output to the filename, including any debug messages that you have in your code base.

\-debugoutput \: Sends console output to the filename, excluding any custom debug messages, but including extra information from the runner for bug reporting.
...
\-minidump: When a fatal exception occurs, this creates a basic minidump file capturing the call stack and any immediate data.

\-fullminidump: Creates a more complete and larger minidump file on fatal exceptions, capturing data sections, call stacks and all thread info.

\-minidumpdir: Without this parameter present, the above minidumps are placed in the [game save area](../Additional_Information/The_File_System.md). However, you may use this parameter to specify the directory where you want the minidumps to be saved.

  Each minidump file is created in the format \.dmp where the uuid is unique for each minidump. This can be loaded into Visual Studio or any other debugger like WinDbg. If the build is a YYC build, its PDB file can be used with the Visual Studio debugger so it uses the symbols from the YYC project.

It's worth noting that the \ in \-output and \-debugoutput can be the same one if desired, as the file is opened and closed on each write.
...
This will open a window similar to the one above. Here you can add into the **Target** path your command line. Simply scroll along to the end of the given path, add a space, and then the parameter string. So, for example, to create a debug file output you would have something like:
...
\-game \: loads the given game file
...
C:\\ProgramData\\GameMakerStudio2\\Cache\\runtimes\\\\\windows\\Runner.exe

Note that the runtime\_version folder will depend on the runtimes you have installed currently. You will also need the path to the \.win file, as this is what we are going to point the runner to. The easiest way to find this is to go to the **Temp** folder (typically located at %localappdata%\\Temp\\GameMakerStudio\\\\\) and just search for \*.win, as that will show you all those available and you can simply pick that which you need to get the full path from.
...
START C:\\ProgramData\\GameMakerStudio2\\Cache\\runtimes\\\\\windows\\Runner.exe \-game \\\\.win
