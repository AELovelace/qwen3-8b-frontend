...
If you select "**Yes**" then the modules for the target will be downloaded and installed.
  ...
  See the [Runtime Feeds Preferences](../Setting_Up_And_Version_Information/IDE_Preferences/Runtime_Feed_Preferences.md) page for more info on how to change which modules are included.

Specific details on compiling for each platform are given below in [Creating A Final Executable Package](Compiling.md#h2).
...
For 2024\.800\+ beta versions of GameMaker, some platforms may also include the "**GMRT**" and "**GMRT VM**" outputs as part of the [GMRT (GameMaker Runtime)](../Settings/Runner_Details/GMRT_(GameMaker_Runtime).md) open beta.
...
The exact procedure and requirements for setting up devices and troubleshooting issues can be found in the appropriate section of the [GameMaker Wiki](https://github.com/YoYoGames/GameMaker-Bugs/wiki).
...
You can click the **Create Executable** button  in the IDE to start the compiler build or select **Create Executable** from [The Build Menu](../IDE_Navigation/Menus/The_Build_Menu.md). Either process will start the build process which will depend on the target platform selected.
...
- You will not be able to create an executable if you're not logged in. Use the  [Account Menu](../IDE_Navigation/Menus/Version_&_Account_Details.md#account_menu) menu to log in before creating an executable.
- For certain platforms, compiling your game project will require that you have set up the correct  **[build tools](#)** (see  ) and also filled in the appropriate [Platform Preferences](../Setting_Up_And_Version_Information/Platform_Preferences.md).
- Depending on your [licence](https://gamemaker.io/en/help/articles/gamemaker-free-version "Which Licence Is Right For Me?"), you may have the option to build your game via command line, allowing you to set up continuous integration for your project and streamline your building and testing process. See [Building \& Testing via Command Line](../Settings/Building_via_Command_Line.md) for more information on command line building.
- The maximum size of the final game package is 4GB (not including [streamed sounds](../The_Asset_Editors/Sounds.md), [dynamic textures](../Settings/Texture_Information/Dynamic_Textures.md) and [Included Files](../Settings/Included_Files.md)). See [tips for reducing the final game size](https://gamemaker.io/en/help/articles/how-to-reduce-your-game-s-size).
...
See tutorials for: [Game](https://gamemaker.io/en/tutorials/publish-to-gxgames-tutorial) / [Live Wallpaper](https://gamemaker.io/en/help/articles/how-to-make-live-wallpapers-with-gamemaker) / [Game Strip](https://gamemaker.io/en/help/articles/how-to-create-game-strips-with-gamemaker)  
...
Please refer to [this page](https://gamemaker.io/en/tutorials/publish-to-gxgames-tutorial) to learn more about the GX.games upload process, and [this page](https://github.com/YoYoGames/GameMaker-Bugs/wiki/GX.Games-GMS2) for setting up the various exports, such as [YYC](../Settings/YoYo_Compiler.md) and Mobile.
- **Windows** \- Compiling for Windows will first request that you choose between creating an *Installer* or a *Zip* package for x64 Windows (with separate options for building for Arm64 Windows). An installer will be a single executable that will install your game, and the Zip file will be a single \*.zip format compressed file with all your game files stored within (the files will need extracted for the game to run). If you check the box marked **Remember Packaging Option** then GameMaker will remember the choice for all future compiles (this can be reset or changed from the [Windows Preferences](../Setting_Up_And_Version_Information/Platform_Preferences/Windows.md)). You can find out more from the [GameMaker Wiki](https://github.com/YoYoGames/GameMaker-Bugs/wiki/Windows-GMS2).  

  

 The Windows target only compiles 64\-bit executables.
- **Ubuntu (Linux)** \- Compiling for Ubuntu will ask you to choose between creating an **.AppImage** or a **.zip** file, containing either an x64 or an ARM executable (however AppImage does not support ARM). You can find out more from the [GameMaker Wiki](https://github.com/YoYoGames/GameMaker-Bugs/wiki/Ubuntu-GMS2).  
...
The type of file you choose will depend on the store that you wish to target, with the \*.aab file being required for Google Play, while the \*.apk file can be used on other stores. You can find out more from the [GameMaker Wiki](https://github.com/YoYoGames/GameMaker-Bugs/wiki/Android-GMS2).
- **iOS** \- Compiling to iOS will create an xarchive file which is then used in Xtools to create the final iOS package. Note that **to compile for iOS you will require an Apple Mac computer running OSX or higher as well as the relevant certificates and permissions**. You can find out more from the [GameMaker Wiki](https://github.com/YoYoGames/GameMaker-Bugs/wiki/iOS-GMS2).
...
"Package as DMG" will build a .dmg installer package which can be used to install the game on a Mac computer. Note that during this build process you will see the installer appear on the Mac, which will disappear once the build process is complete.  
 ...
 "Package as Zip" will build either an \*.app file or a \*.pkg file, depending on whether you want to later upload it to the Mac App Store or not. As with iOS **you will require an Apple Mac computer running OSX or higher as well as the relevant certificates and permissions**. You can find out more from the [GameMaker Wiki](https://github.com/YoYoGames/GameMaker-Bugs/wiki/MacOS-GMS2).
- **Reddit** \- For a compilation guide, see the [GameMaker Wiki](https://github.com/YoYoGames/GameMaker-Bugs/wiki/). Note that you cannot use debug mode or create an executable when targetting Reddit, as compilation for the Reddit SDK happens through the "Run" (F5\) button.
