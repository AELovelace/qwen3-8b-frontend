...
- **Round**: This takes a decimal number and rounds it up *or* down to the nearest integer number. In the special case where the number supplied is exactly half an integer (for example, 1\.5, 17\.5, \-2\.5, etc...) the number will be rounded to
...
- **Ceil**: This will take a decimal value and round it *up* to the nearest integer. Unlike round (above) this does not take into consideration the decimal part of the value, so a number like 5\.0000001 would still be rounded up to 6\. It is worth
 noting that if you are using the Random action [Get Random Number](../Random/Get_Random_Number.md) to generate a random decimal value and then calling this action to round it up, then you *may* get unexpected results. The random
...
- **Floor**: This will take a decimal value and round it *down* to the nearest integer value. Unlike round (above) this does not take into consideration the decimal part of the value, so a number like 5\.9999999 would still be floored down to
 5\.
