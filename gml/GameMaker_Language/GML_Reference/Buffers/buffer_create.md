...
The function allocates a portion of memory for the buffer, which can then be used to store different types of data (specified when you write to the buffer using the [buffer\_write](buffer_write.md), [buffer\_poke](buffer_poke.md) or [buffer\_fill](buffer_fill.md) function).
...
- Floats of up to 64bits should be aligned to 8 bytes.
  ...
  You cannot create buffers that are 2 Gibibytes (2,147,483,648 bytes) or larger.
...
| --- | --- | --- |
| size | [Real](../../GML_Overview/Data_Types.md) | The size (in bytes) of the buffer. |
| type | [Buffer Type Constant](buffer_create.md) | The type of buffer to create (see the constants list above). |
| alignment | [Real](../../GML_Overview/Data_Types.md) | The byte alignment for the buffer |
