...
- On Android you supply the permission to request as a string using the format "android.permission.\", so to request, for example, the RECORD\_AUDIO permission you would call:

os\_request\_permission("android.permission.RECORD\_AUDIO");

- On GX.games you can use this function to request access to orientation information, though only on certain browsers (see [Device Input](../Game_Input/Device_Input/Device_Input.md)). This permission request has to be made in response to a user press, so will only take place the first time the app is clicked after the call to os\_request\_permission is made. In this case you request the permission "DeviceMotion":

os\_request\_permission("DeviceMotion");

  You can request multiple permissions at once by passing multiple arguments into the function.
...
- "**\**": This will be a permission requested by the function, as a string. You will get keys for all the permissions you requested in the original function call.  
...
| --- | --- | --- |
| permissions | [String](../../GML_Overview/Data_Types.md) | The permission(s) to request. Request multiple permissions by adding more arguments. |
