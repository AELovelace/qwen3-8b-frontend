...
The Debug Overlay is an in\-game overlay that displays various real\-time debug information about your game.
  ...
  The Debug Overlay is not supported on the HTML5 target platform.
...
You can define your own, custom debug views in The Debug Overlay using [dbg\_view](dbg_view.md). Debug views can have sections added to them using [dbg\_section](dbg_section.md), which you can add controls to such as sliders, string input boxes, etc. These allow you to change the values of variables through references created using [ref\_create](../Variable_Functions/ref_create.md). The **DebugView** option under the **Debug** menu determines if custom views are visible. All debug views are listed under the **Views** menu and can have their visibility toggled here.

You can use the keyboard and mouse for input on the overlay, as well as a gamepad if one is connected. It will automatically use the first connected gamepad it finds for input.

### Dragging and Docking Windows

All windows of the Debug Overlay can be dragged by clicking and holding   on the title bar or by performing the same action anywhere else on empty window space.

The built\-in windows moved into a single window

Clicking   on the title bar and dragging the window allows you to "dock" the window to other windows in a variety of ways: 
...
- **Scroll**: Time spent processing scrolling backgrounds in [rooms](../Asset_Management/Rooms/Rooms.md), and [video](../Drawing/Videos/Videos.md)
...
All debug messages that you show using [show\_debug\_message](show_debug_message.md) and [show\_debug\_message\_ext](../Strings/show_debug_message_ext.md) are also shown here.
...
- The console also supports autocomplete that you can access by pressing the Tab key, which will be shown when the cursor is on a word with more than 2 characters. Up and Down keys select the option in the dropdown box and Escape closes the autocomplete:
  ...
  While you type commands in the console input, GameMaker continues to trigger events based on the [Keyboard Input](../Game_Input/Keyboard_Input/Keyboard_Input.md) that you provide, meaning your game still receives input.
...
- Anything else not following the above rule is treated as a string
  ...
  All scripts are executed within the scope of the [global](../../GML_Overview/Variables/Global_Variables.md#global) struct.
...
The list at the bottom shows you each sound "source" that is currently playing or may potentially play a sound (including individual sources from [sync groups](../Asset_Management/Audio/Audio_Synchronisation/Audio_Synchronisation.md)). These appear coloured depending on the state of the source:
...
This displays information about all the [Texture Groups](../../../Settings/Texture_Groups.md) in the game.

You can enable/disable automatic loading of Texture Groups (when a sprite is drawn from an unloaded Dynamic Texture Group), asynchronous loading of textures (only to be used for specific debugging) and allows you to load/unload Dynamic Texture Groups that were created at runtime.
...
It shows the textures generated by GameMaker for all [Texture Groups](../../../Settings/Texture_Groups.md), as well as couple of other textures, such as the fallback texture. Custom [Surfaces](../Drawing/Surfaces/Surfaces.md) that you create as well as the [application\_surface](../Drawing/Surfaces/application_surface.md) are also shown here.
  ...
  When you access [Dynamic Textures](../../../Settings/Texture_Information/Dynamic_Textures.md) in the texture viewer that are unloaded and still on disk they will be loaded into memory and VRAM so they can be displayed.

Drag the **Texture Index** slider to select the texture. For every texture, the following information is shown: 

- **width**: The width of the texture in pixels.
- **height**: The height of the texture in pixels.
- **group**: The name of the texture group this texture belongs to. Shows \ if the texture doesn't belong to a texture group.
- **index in group**: The index of the texture page in its texture group. A texture group can be stored on multiple texture pages when all the visual assets in it don't fit on a single texture page. Sprites marked as on a [Separate Texture Page](../../../Settings/Texture_Information/Texture_Pages.md#h) also go on their own texture page.
- **num mips**: The number of mipmap levels used by this texture. See [Mipmapping](../Drawing/Mipmapping/Mipmapping.md) for more info.

### FlexPanel

This shows the [Flex Panel Preview](../Flex_Panels/Flex_Panels.md#h1) window, with a JSON editor on the left and a preview of the generated layout on the right.

### DebugView

This toggles the visibility of all custom debug views created using [dbg\_view](dbg_view.md).

## Views

This menu lists all the custom debug views that you've created using [dbg\_view](dbg_view.md).
...
Each menu item shows the name of the debug view with a check mark on the right if the view is set to be visible. Clicking the menu item toggles the visibility of this debug view.
  ...
  The Debug menu's [DebugView](The_Debug_Overlay.md#debug_view) setting must be enabled for any custom debug views to be shown.

### Creating Debug Views

A customdebug view is created using [dbg\_view](dbg_view.md):
...
References can be created to all kinds of variables. Each control supports a reference to one or more datatypes.
  ...
  Variables must exist within a [struct](../../GML_Overview/Structs.md#struct) or [instance](../Asset_Management/Instances/Instances.md) as a debug view is declared once, so local variables *cannot* be displayed.
...
Controls can later be deleted using [dbg\_control\_delete](dbg_control_delete.md).

Creating a new view or section sets it as the currently active view/section, which is where any new controls go. You can change the currently active view and section later with [dbg\_set\_view](dbg_set_view.md) and [dbg\_set\_section](dbg_set_section.md) respectively.
...
When the mouse is over any menu or window of the Debug Overlay or when the Debug Overlay expects keyboard input, GameMaker will not trigger any keyboard or mouse events. Functions such as [keyboard\_check](../Game_Input/Keyboard_Input/keyboard_check.md) and [mouse\_check\_button](../Game_Input/Mouse_Input/mouse_check_button.md) still work the same, however. If you want to get the same result in this case, you can use the values returned by [is\_keyboard\_used\_debug\_overlay](is_keyboard_used_debug_overlay.md) and/or [is\_mouse\_over\_debug\_overlay](is_mouse_over_debug_overlay.md): 
...
if (!is\_keyboard\_used\_debug\_overlay() \&\& keyboard\_check(vk\_up))  
 ...
 {  
  ...
  The keyboard is considered "used" by the Debug Overlay when the cursor is on a textbox control and also when you drag a window (by clicking and holding the left mouse button).
...
- [dbg\_add\_font\_glyphs](dbg_add_font_glyphs.md)
- [dbg\_get\_gamepad\_input](dbg_get_gamepad_input.md)

### Views

- [dbg\_view](dbg_view.md)
...
- [dbg\_section\_delete](dbg_section_delete.md)
- [dbg\_set\_view](dbg_set_view.md)
- [dbg\_set\_section](dbg_set_section.md)
