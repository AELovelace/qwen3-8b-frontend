...
This function generates a custom debug event that will be shown in the Graph View of [The Debugger](../../../IDE_Tools/The_Debugger.md) when a game is being run in [Debug Mode](../../../Introduction/Debugging.md). If you require messages to be displayed when *not* in debug mode, use [show\_debug\_message](show_debug_message.md).
...
- "\Count": The number of resources of this type that exist when the function is called
- "\MaxIndex": The maximum valid integer index that exists for the resource
- "\ArrayAllocatedSize": The highest number of resources that were ever created for the given resource type
...
| --- | --- | --- |
| string | [String](../../GML_Overview/Data_Types.md) | The custom debug event string to use or one of the five reserved strings. |
| silent | [Boolean](../../GML_Overview/Data_Types.md) | This parameter makes the function silently return the result to the runner in a struct rather than printing the value to the console. Defaults to false. |
