...
This checks for collisions against instances of obj\_rock and obj\_bush in the same function call.
  ...
  It is recommended to use the parenting method for multiple collision checks, as you can use the same parent in multiple function calls, without having to keep track of an array.
...
- A function like [instance\_place](../../Asset_Management/Instances/instance_place.md) or [collision\_circle](collision_circle.md) (and other equivalents) may return 0 if a collision was found with a Tile Map having the ID 0\. This means you should not use the return value as a boolean condition. Instead, check whether the return value is not [noone](../../../GML_Overview/Instance Keywords/noone.md), meaning a collision was found.
...
These bounding boxes use an inclusive system, i.e. they include the bottom\-most and right\-most edges. This means that the bounding box for a 16x16 collision mask is generated from (0\.0, 0\.0\) to (16\.0, 16\.0\) (relative to the instance).
  ...
  You can get these values using the [bbox\_left](../../Asset_Management/Sprites/Sprite_Instance_Variables/bbox_left.md), [bbox\_top](../../Asset_Management/Sprites/Sprite_Instance_Variables/bbox_top.md), [bbox\_right](../../Asset_Management/Sprites/Sprite_Instance_Variables/bbox_right.md) and [bbox\_bottom](../../Asset_Management/Sprites/Sprite_Instance_Variables/bbox_bottom.md) variables.

  Since the bounding box is inclusive (as explained above), it will always extend 1 pixel beyond its bottom\-right corner compared to the sprite's bounding box. For example, a 16x16 square sprite's [right edge offset](../../Asset_Management/Sprites/Sprite_Information/sprite_get_bbox_right.md) will be 15\.0 (the 16th pixel when starting from 0\), however an instance using this sprite will give you a [bbox\_right](../../Asset_Management/Sprites/Sprite_Instance_Variables/bbox_right.md) of 16 (\+ its X position).

For two instances to be in collision, their bounding boxes have to overlap. Collision checks are performed in room coordinates. At a pixel level, this means that an overlap is counted **when the centre of that pixel is covered**.
...
For example, if you're trying to collide with a bounding box covering the area from (0\.0, 0\.0\) to (16\.0, 16\.0\), the edge of your mask has to touch the area between  (0\.5, 0\.5\) and (15\.5, 15\.5\) for a collision to be counted.
  ...
  This only applies when an instance (and hence its mask) is involved in a collision check. If you do a collision check of a shape against another shape (e.g. [rectangle\_in\_rectangle](rectangle_in_rectangle.md)) or a shape against a non\-instance mask (e.g. Tile Maps with [collision\_rectangle](collision_rectangle.md)), then pixel centres don't have to be covered for a collision to be counted.

  The functions [collision\_point](collision_point.md) and [collision\_line](collision_line.md) are also exceptions, where they don't have to cover the pixel centre but can be checked anywhere inside the pixel. In the above example, a point check at (15\.99, 15\.99\) would return true, but (16\.0, 16\.0\) would not.

  When using views that magnify a portion of the room, some pixels in the room can cover multiple pixels in the viewport. While these pixels might appear to overlap visually, they may not actually be in collision in room coordinates.
...
- [instance\_place\_list](../../Asset_Management/Instances/instance_place_list.md)
...
The following functions can be used for this (as well as other functions detailed in the sections relating to [**Moving Around**](../Movement/Movement.md) and [**Instances**](../../Asset_Management/Instances/Instances.md)). These collision functions all work slightly differently but they maintain three common arguments which we will explain here:
...
- [collision\_circle\_list](collision_circle_list.md)
...
- [collision\_line\_list](collision_line_list.md)
- [collision\_point](collision_point.md)
- [collision\_point\_list](collision_point_list.md)
- [collision\_rectangle](collision_rectangle.md)
...
- [rectangle\_in\_rectangle](rectangle_in_rectangle.md)
