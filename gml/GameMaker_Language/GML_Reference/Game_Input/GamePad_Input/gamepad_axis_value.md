...
Note that the returned value is normalised to take into consideration the value set for the axis deadzone (using the function [gamepad\_set\_axis\_deadzone()](gamepad_set_axis_deadzone.md)), and so will always be between \-1 and 0, and 0 and 1\. For example, setting the deadzone to 0\.2 will mean that pushing the stick right will only start returning a value from 0 \- 1 when the *raw* axis value is over 0\.2 \- so for example, when the raw value is 0\.2, the return value will be 0, when the raw value is at 0\.5, the return value will be 0\.375, or if the raw value is 0\.9, then the return value would be 0\.875\.
...
| axisIndex | [Gamepad Axis Constant](Gamepad_Input.md) | The axis index to check (see the [constants list](Gamepad_Input.md)). |
