...
Since the function doesn't return anything, you can use the function [ds\_map\_replace](ds_map_replace.md) to check if the key value has been replaced or a new key has been created.

 
  This function is the same as using the [DS map accessor](../../../GML_Overview/Accessors.md) to set/create a map key/value pair.
...
| key | [Any](../../../GML_Overview/Data_Types.md#variable) | The key to set. |
| value | [Any](../../../GML_Overview/Data_Types.md#variable) | The value to set the key to. |
...
The above code first creates a basic struct struct\_key. It then creates a new DS map using [ds\_map\_create](ds_map_create.md#h) and stores it in a variable map\_key\_value\_pairs.
...
Finally, the three values are looked up using the DS map accessor ? and shown using [show\_debug\_message](../../Debugging/show_debug_message.md).
