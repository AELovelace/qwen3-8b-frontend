...
| [Network Config Constant](network_set_config.md) | | |
| --- | --- | --- |
| Constant | Description | Setting |
| network\_config\_connect\_timeout | Set a connection timeout value | A value in milliseconds |
| network\_config\_use\_non\_blocking\_socket | Tell GameMaker not to block on connect. | 0 \= block, 1 \= don't block |
| network\_config\_enable\_reliable\_udp | Enables the "reliable UDP" protocol for an existing UDP socket | The socket ID to target |
| network\_config\_disable\_reliable\_udp | Disables the "reliable UDP" protocol for an existing UDP socked. | The socket ID to target |
| network\_config\_avoid\_time\_wait | Sets the SO\_LINGER timeout value to 0 for an exisiting TCP socket | The socket ID to target |
| network\_config\_enable\_multicast | Enables multicast support for UDP | The socket ID to target |
| network\_config\_disable\_multicast | Disables multicast support for UDP | The socket ID to target |
| network\_config\_message\_size\_limit | Sets the size limit on incoming messages | The size limit in bytes |
...
The second option (network\_config\_use\_non\_blocking\_socket) means that the network\_connect\_\* functions (non\-async) will all return a socket\_id instantly, *but you can't send or receive on that socket until you've received an async network event*. The event triggered will have the "type" key set to network\_type\_non\_blocking\_connect (you can find further details from the page on the [Networking Asynchronous Event](../../../The_Asset_Editors/Object_Properties/Async_Events/Networking.md)). This is a **global** setting as GameMaker doesn't support a mixture of blocking and non\-blocking in one application.
...
- It is possible to have two separate UDP sockets, one with reliable UDP enabled and one with reliable UDP disabled for the other.
...
The option network\_config\_message\_size\_limit sets the incoming message size limit for a given socket. This limit defaults to 64MiB for sockets created by [network\_create\_socket](network_create_socket.md) or [network\_create\_socket\_ext](network_create_socket_ext.md) and 4MiB for sockets created by a client connecting to a server.

Note that limiting the receive size of messages for WebSocket client sockets on Windows is not supported.

 

#### Syntax:

network\_set\_config(config\_value, value1, \[value2])

| Argument | Type | Description |
| --- | --- | --- |
| config\_value | [Network Config Constant](network_set_config.md) | The config constant to set (refer to the table at the top of this page). |
| value1 | [Any](../../GML_Overview/Data_Types.md#variable) | The setting value of the config, or the socket when the constant is network\_config\_message\_size\_limit |
| value2 | [Any](../../GML_Overview/Data_Types.md#variable) | The size limit in bytes, when the constant is network\_config\_message\_size\_limit |
...
#### Example 2: Message Size Limit

network\_set\_config(network\_config\_message\_size\_limit, socket, 4096\);

The above code sets the message size limit on a socket stored in a variable socket to 4096 bytes.
