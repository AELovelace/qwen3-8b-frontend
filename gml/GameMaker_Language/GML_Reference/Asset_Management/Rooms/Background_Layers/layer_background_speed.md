...
| --- | --- | --- |
| background\_element\_id | [Background Element ID](layer_background_get_id.md) | The unique ID value of the background element to change |
| speed | [Real](../../../../GML_Overview/Data_Types.md) | The speed multiplier/value to use for the element |
...
The above code will get the layer handle for the layer named "Background\_water" and then use that to get the ID of the background element on that layer. This ID is then used to change the element's image speed.
