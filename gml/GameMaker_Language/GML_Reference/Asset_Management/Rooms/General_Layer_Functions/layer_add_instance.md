...
You supply the layer handle (which you get when you create the layer using [layer\_create](layer_create.md)) or the layer name (as a string \- this will have a performance impact) and the handle of the instance to move between layers. For example, say your player is on a layer that is at a lower depth than another layer and you want it to appear behind the layers between the two. You can use this function to switch the player from the foreground layer to the background layer and make it appear behind the other layers being drawn.
  ...
  This function cannot be used to add a new instance to a layer. You **must** have created the instance previously and stored its reference in a variable.

 This function is only valid within the scope of the current room and cannot be used when the scope has been changed using the function [layer\_set\_target\_room](layer_set_target_room.md).
...
| --- | --- | --- |
| layer\_id | [String](../../../../GML_Overview/Data_Types.md) or [Layer](layer_get_id.md) | The handle of the instance layer to target (or the layer name as a string) |
| instance\_id | [Object Instance](../../Instances/Instance_Variables/id.md) | The handle of the instance to move over to the target layer |
