...
This function can be used to enable or disable anti\-aliasing (AA) for fonts added using [font\_add()](font_add.md). This function needs to be called *before* you add any fonts and setting it to true will enable AA, and setting it to false will disable it. By default AA is enabled. Note that this will have no effect on fonts that have been added before the function was called, and the function only needs to be called once when the font is added, and not every draw/step that the font is being used.
...
| --- | --- | --- |
| enable |  | Whether to enable (true) or disable (false) AA for added fonts. |
