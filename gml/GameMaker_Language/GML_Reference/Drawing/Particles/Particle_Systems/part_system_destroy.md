...
The function should always be called when the system is no longer needed, like at the end of a room, or in the Clean Up event of an instance, otherwise you could end up with particles appearing in later rooms and have no way to remove them as well as a memory leak which will eventually crash your game.
  ...
  This function does not destroy the particle types used by the particle system. Types created with [part\_type\_create](../Particle_Types/part_type_create.md) should be destroyed using [part\_type\_destroy](../Particle_Types/part_type_destroy.md).
...
| --- | --- | --- |
| ind | [Particle System Instance](part_system_create.md) | The particle system to destroy. |
