...
In this way you can better control texture page memory usage on platforms with low memory issues. Note that textures pages are created on demand from the WAD, and so you can either call this at the start of the game to load all textures scaled, or you can call it at specific times to load specific textures, in which case you'd probably want to call [draw\_texture\_flush](draw_texture_flush.md) before calling this function, then use the pre\-fetch functions to get the correct texture pages into memory.
...
| --- | --- | --- |
| pow2integer | [Real](../../../GML_Overview/Data_Types.md) | The scale factor to use (1, no scale, 2, half scale, 4, quarter scale, etc.) |
