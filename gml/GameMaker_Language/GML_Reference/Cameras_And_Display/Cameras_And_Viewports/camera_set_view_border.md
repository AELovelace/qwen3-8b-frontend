...
These values will only be used when the view camera has been assigned an instance target to follow (either in the Room Editor \- see [Cameras And Viewports](../../../../The_Asset_Editors/Room_Properties/Room_Properties.md#h) \- or using the function [camera\_set\_view\_target](camera_set_view_target.md)) and relate to how far from the border of the view the instance needs to be before the view will update its position to follow.
...
You give the unique camera ID value (as returned by the different [camera\_create](camera_create.md) functions) and then give the x border size and the y border size (horizontal and vertical).
...
| x\_border | [Real](../../../GML_Overview/Data_Types.md) | The border (in pixels) for the view camera along the horizontal axis |
| y\_border | [Real](../../../GML_Overview/Data_Types.md) | The border (in pixels) for the view camera along the vertical axis |
...
The above code will set the view camera border for the camera assigned to viewport\[0].
