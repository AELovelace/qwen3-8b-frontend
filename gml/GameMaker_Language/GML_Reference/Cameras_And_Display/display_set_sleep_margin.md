...
Note that even if the timing method is set to use vsync timing, the sleep margin will have some effect over the rendering of the game, and so you should take care when setting this value. For more information on display timing, please see [display\_set\_timing\_method()](display_set_timing_method.md).
 ...
 In addition to the Sleep Margin, you can further control your performance and power consumption on Windows by adjusting the [thread scheduler's resolution](../OS_And_Compiler/scheduler_resolution_set.md) at runtime.
...
| --- | --- | --- |
| milliseconds | [Real](GameMaker_Language/GML_Overview/Data_Types.md) | The number of milliseconds for the sleep margin |
