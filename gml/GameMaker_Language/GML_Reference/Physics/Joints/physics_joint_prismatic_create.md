...
This function creates a prismatic joint between two physics instances.
...
We have two instances joined at the anchor point, with an axis for movement defined from the vector the w\_axis\_x/w\_axis\_y coordinates relative to the (0, 0\) coordinates of the physics world (so an x component of 0 and a y component of 1 (0, 1\) would make the joint a vertical prismatic joint). This joint can then *only* move relative to this axis, like a spring or a piston. If you set the lower or upper trans limit, you are basically limiting the amount of movement along this axis, where the 0 position is the exact spot that you defined with w\_anchor\_x/w\_anchor\_y, so a negative value would go to the "left" of that point along the axis and a positive value to the "right" as shown in the following diagram (realise that "left" and "right" are relative terms!):

You can also define the joint as having a motor or not. This means that when un\-influenced by a collision the joint will move in a direction along the axis. this direction is defined by the motor speed, with a positive number being towards the axis coordinates ("right") and a negative number being towards the anchor point ("left"). The max\_motor\_force argument is for limiting the speed of the movement so that you don't get a perpetually accelerating motor and to limit the influence that a collision may have on the movement. In this way you can use a joint motor to simulate joint friction by setting the joint speed to zero and maximum force to some small, but significant value. The motor will try to prevent the joint from moving, but will yield to a significant load.
...
| w\_anchor\_x | [Real](../../../GML_Overview/Data_Types.md) | The x coordinate where the joint is anchored, within the game world |
| w\_anchor\_y | [Real](../../../GML_Overview/Data_Types.md) | The y coordinate where the joint is anchored, within the game world |
| w\_axis\_x | [Real](../../../GML_Overview/Data_Types.md) | The x component of the axis vector |
| w\_axis\_y | [Real](../../../GML_Overview/Data_Types.md) | The y component of the axis vector |
| lower\_trans\_limit | [Real](../../../GML_Overview/Data_Types.md) | The lower permitted limit for the joint movement |
| upper\_trans\_limit | [Real](../../../GML_Overview/Data_Types.md) | The lower permitted limit for the joint movement |
| limit | [Boolean](../../../GML_Overview/Data_Types.md) | Whether to limit the movement of the joint (true) or not (false) |
| max\_motor\_force | [Real](../../../GML_Overview/Data_Types.md) | Sets the maximum movement speed for the motor |
| motor\_speed | [Real](../../../GML_Overview/Data_Types.md) | This is the speed at which the motor should move |
| motor | [Boolean](../../../GML_Overview/Data_Types.md) | Whether the motor should be active (true) or not (false) |
