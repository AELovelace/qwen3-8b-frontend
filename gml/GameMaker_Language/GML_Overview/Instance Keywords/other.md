...
Also see: [Legacy Values](../Instance_Keywords.md#h)
...
The page on [self](self.md) contains a section on [When 'self' changes](self.md#h).
...
This behaviour can be enabled by the [Legacy Other Behaviour](../../../Settings/Game_Options.md#legacy_other_behaviour) game option.
...
However, you do not need to use other to read variables from the instance as any variables you reference directly will be read from that instance's scope, as described [in this section](../Structs.md#inst_in_struct) of the manual. You would only need to use this if you wanted to store a reference to that instance's struct.

### 'other' as a reference

The page on [self](self.md) contains a section on ['self' as a reference](self.md#h1) with an example, which you should read before this section. This will cover that same example and how it would affect the other reference.
