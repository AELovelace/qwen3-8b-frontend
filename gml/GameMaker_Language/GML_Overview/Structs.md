...
A **struct** is a variable that holds a collection of other variables. The variables can be of any [data type](Data_Types.md) previously mentioned and are stored in no particular order in the struct. They can be read from and written to after the initial struct declaration, and you can also add more variables to a struct after it has been declared.

  There are subtle differences between Structs \& Constructors and [Objects And Instances](../../Quick_Start_Guide/Objects_And_Instances.md). See [Constructors \& Structs vs. Objects \& Instances](Structs.md#constructors_structs_vs_objects_instances) for more info.

The variables used in a struct should follow the usual variable naming scheme, i.e.: they cannot start with a number and should only be made up of alphanumeric characters and the underscore "\_" character, and also note that the contents of a struct are *independent of the instance or function that created it*, and as such you can \- if you wish \- use built\-in variable names such as [image\_index](../GML_Reference/Asset_Management/Sprites/Sprite_Instance_Variables/image_index.md) or [x](../GML_Reference/Asset_Management/Instances/Instance_Variables/x.md) and [y](../GML_Reference/Asset_Management/Instances/Instance_Variables/y.md).
...
In the above code the struct member variables x and y are being set to the values held in the instance variables [x](../GML_Reference/Asset_Management/Instances/Instance_Variables/x.md) and [y](../GML_Reference/Asset_Management/Instances/Instance_Variables/y.md), since the right\-hand side of the colon : refers to the instance that is defining the struct. It is worth noting that this means you *cannot* use struct member variables for defining subsequent variables within the struct declaration. For example, the following would give you an error:
...
The error occurs because the variables a and b are actually being evaluated at the scope of whatever is defining the struct (they are on the right\-hand side of the colon :), and are *not* the ones being defined within the struct itself.
...
A last way to access data in a struct is by using the struct accessor [$](Accessors.md#$). This allows access to struct variables using strings, for both reading: 
...
The variable v2 will now contain a struct with the variables x and y and the [static](Functions/Static_Variables.md) [method variable](Method_Variables.md) Add.
  ...
  You cannot call a constructor function without the new keyword and doing so will throw a fatal error. You can, however, call a constructor without the new keyword on a previously created struct with [script\_execute](../GML_Reference/Asset_Management/Scripts/script_execute.md) or [script\_execute\_ext](../GML_Reference/Asset_Management/Scripts/script_execute_ext.md).
...
Constructor functions defined in [Script Assets](../../The_Asset_Editors/Scripts.md) are assigned [tags](../../Introduction/The_Asset_Browser.md#h) at compile time:

- Every constructor function gets a @@constructor tag
- Every inheriting/child constructor function gets a @@parent\=\ tag where \ is the name of the parent constructor function

This can be used to check whether a script function is a constructor, and if so, whether it has a parent constructor.

These tags can be retrieved using [asset\_get\_tags](../GML_Reference/Asset_Management/Assets_And_Tags/asset_get_tags.md) by passing in the constructor function's handle:

function parent\_func() constructor  

 {}  

  

 function child\_func() : parent\_func() constructor  

 {}  

  

 show\_debug\_message( asset\_get\_tags(parent\_func, asset\_script) );   // Print tags for parent\_func  

 show\_debug\_message( asset\_get\_tags(child\_func, asset\_script) );    // Print tags for child\_func
 

This will print the following to the Output Log:

\[ "@@constructor" ]  

 \[ "@@constructor","@@parent\=parent\_func" ]

This shows that parent\_func is a constructor and child\_func is a constructor with parent\_func as its parent.

Inversely, all defined constructor functions can be retrieved using [tag\_get\_asset\_ids](../GML_Reference/Asset_Management/Assets_And_Tags/tag_get_asset_ids.md):

show\_debug\_message( tag\_get\_asset\_ids("@@constructor", asset\_script) )

This prints the available constructor functions to the Output Log:

\[ ref script parent\_func,ref script child\_func ]

## String Output

One final thing to mention about structs is that you can change what is output to the console from them for debugging. By default, calling the function [show\_debug\_message](../GML_Reference/Debugging/show_debug_message.md) on a struct will output the contents of the struct (as shown above). However, it's possible to customise this message by adding a specifically named method to the struct called toString:
...
Now when the [show\_debug\_message](../GML_Reference/Debugging/show_debug_message.md) function is called, the toString method will be used to generate the output and \- with the above example \- you'll get:
...
Finally, there are a number of runtime functions that you can use on structs to get the variables they contain as well as a few other things. You can find them under [Struct Functions](../GML_Reference/Variable_Functions/Variable_Functions.md#struct_functions).
