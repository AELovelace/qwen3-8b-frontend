...
Constants in GameMaker are global, i.e. they are accessible from any [Variable Scope](../Variables_And_Variable_Scope.md#h) in the game, and note that unlike [Global Variables](Global_Variables.md), they do not require use of the global. prefix.
...
The enum entries can only be **integer numbers** or **[expression](#)s with previous enums or built\-in function calls that evaluate to an integer number**, and by default are numbered from 0 upwards, so our example given above would default to RED \= 0, ORANGE \= 1, YELLOW \= 2, etc.
...
The colour\_value variable would now hold the value 100 (20 \* 5\).
  ...
  Enum values are stored as int64s, so running [is\_real](../../GML_Reference/Variable_Functions/is_real.md) on them will return false.
...
\#macro TOTAL\_WEAPONS \= 10;  
 ...
 Removing the equal sign and colon will make it a valid macro definition.  
  ...
  The recommended GML naming conventions recommend UPPERCASE\_NAMING for constants, however you may use lower case or any other naming conventions you like.
...
You can define a macro anywhere in your code or in a script and it will be *pre\-compiled* and included in your game as if it was there from the start, but we recommend that you create a dedicated script asset and define all your macros in there. It will be easier to organise and debug later!
  ...
  It's recommended to define macros in a Script asset (outside of a function). Placing macro declarations inside statements or literals (e.g. in an array) may cause syntax errors to appear in the IDE.
...
| --- | --- |
| pointer\_null | This constant indicates that the pointer is not pointing to anything meaningful (the same as NULL in C\+\+ or null in C\#).  This value is [falsy](../Data_Types.md#). |
| pointer\_invalid | This constant simply means that the value is not a valid pointer |
| undefined | This constant is returned when a function has to return *something* but has no appropriate or "correct" value to return.  This value is [falsy](../Data_Types.md#). |
...
See [Equality Table](../../../Additional_Information/Type_Tables.md#h) for information on equality comparisons for a few of the constants listed above.
