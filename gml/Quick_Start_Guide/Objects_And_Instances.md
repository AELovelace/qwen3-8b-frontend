...
So, **the object is the template for the instance, and the instance is what we place in a room to make our game**.
  ...
  The detailed differences between objects and instances at runtime are described on [Objects vs. Instances](../Additional_Information/Objects_vs_Instances.md).

## Object Editor

To create a new object asset, it's the same as for every other asset as you simply right click  anywhere in the Asset Browser and select **Create \-\> Object**.
...
In general, for an object to draw anything and register collisions with instances of other objects, it needs to have a sprite assigned to it, which is added using the  button. This will open the **Asset Explorer** where you can select the sprite to use (you can also drag sprites from the Asset Browser and drop them onto the Object Editor to assign them):
...
Once you select a sprite for the object, you can click on the *Edit Sprite* or *Edit Image* buttons to edit either the sprite asset's properties, or the images inside the sprite, respectively. This can be faster than first finding the sprite in the Asset Browser and then indicating you want to edit it.
