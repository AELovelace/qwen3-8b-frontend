...
In this example, we're going to blend different colours with the sprite while a key is pressed and held down, and so you'll need to open (or create) an object, assign it a sprite, and then give the object a **Key Down \ Event**.
...
Assign the first sprite ("spr\_One" with the center origin) to the object you have created and then add a **Create Even**t. In the Create Event add the following GML Visual or GML:
...
We also want to add a **Keyboard Down \ Event** to the object, as we'll be using that to increment the score while you hold the Space key.
...
This generates a random number from 0 to 100 and *adds* that to the "player\_score" variable (in GML Visual, "**Relative**" is marked, making it an addition operation instead of assignment).
...
Add a single instance of this object to your room now and then press the **Play** button . When the game runs press the \ key and you should see the score value increase.
...
Next we want to add a **Keyboard Pressed \ Event** to the object, as we'll be using that to change the number of lives every time the Enter key is hit:
...
If you haven't already added an instance of this object to a room, go ahead and add it now (only one!), then press the **Play** button . Once the game is running press the \ key various times to see the lives change.
...
Add an instance of this object to a room if you haven't already done so (only one, though!), and then press the **Play** button . Once the game is running press the \ and \ keys various times to see the health change.
