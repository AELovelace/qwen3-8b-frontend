...
Once you have loaded your file you can preview the sound by using the play button which will loop it continuously. This is very handy as it means you can make changes to the volume, attributes and target options, and hear what effect it will have on the sound. There is a stop button too, so you can stop the preview at any time.

  Use [The Inspector](../IDE_Tools/The_Inspector.md) to view the waveform for the selected sound.
...
If you have selected *streamed* audio then this option will be **disabled**, as streamed sounds are not packaged in the same way as other sounds and will be not be loaded into memory, but instead streamed directly from the storage medium.

  While the rest of the audio properties are configuration specific, the audio group setting is *not* and a sound will belong to the same group on *all configurations that you use*.

[Export Directory](#)

You can specify an export directory that is relative to the game bundle directory. The audio file will be placed inside that directory when the game is exported. If this field is empty, the audio file will be placed at the root of the game bundle.

Note that the [Audio Group](../Settings/Audio_Groups.md) assigned to this sound can also have its own export directory set, which will override the directory set for the individual sound asset.

## Inspector

When a Sound asset is selected, [The Inspector](../IDE_Tools/The_Inspector.md) will show the waveform of the audio along with information about its size:

"**Original Size**" shows the size of the original audio file and "**Export Size**" shows the final size of the sound in the game package, which changes depending on the attributes and target options selected. "**Size Ratio**" shows the final size as a percentage of the original size of the audio file.

Sizes are shown in KB and hovering over them reveals the size in bytes in a tooltip. The  button hides the size info and the  button copies it to your clipboard.
