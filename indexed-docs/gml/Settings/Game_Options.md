...
- **Deprecated Behaviours**: This section contains checkboxes that can be used to enable deprecated behaviours for the current project. All of these are disabled by default for new projects and it is recommended to keep them disabled unless specifically needed for an older project that relies on a deprecated behaviour.
	...
	- **Legacy Other Behaviour**: Checking this enables legacy behaviour of [other](../GameMaker_Language/GML_Overview/Instance Keywords/other.md). See [Legacy other Behaviour](../GameMaker_Language/GML_Overview/Instance Keywords/other.md#legacy_other_behaviour).
	- **Allow instance\_change**: Allow use of the deprecated [instance\_change](../GameMaker_Language/GML_Reference/Asset_Management/Instances/instance_change.md) and [position\_change](../GameMaker_Language/GML_Reference/Movement_And_Collisions/Collisions/position_change.md) functions.
	- **Enable Copy on Write Behaviour for Arrays**: This enables the deprecated [Copy on Write](../GameMaker_Language/GML_Overview/Arrays.md#copy_on_write) array behaviour.
	- **Legacy primitive drawing behaviour**: GameMaker 2024\.14 introduced a change to correct the coordinates used to draw primitives and shapes so they more accurately reflect the coordinates passed to the drawing function. Enabling this option changes the current project to use the old drawing behaviour.
- **Automatically remove unused assets when compiling**: This option enables automatic removal of unused assets when compiling. When enabled, any unused assets that are removed will be logged in the compile output of the project. This setting is enabled by default. You can mark tags as used with [gml\_pragma](../GameMaker_Language/GML_Reference/OS_And_Compiler/gml_pragma.md)("MarkTagAsUsed", \) and any assets with the specified tags will not be stripped when this option is enabled. This option does not affect assets used from the [Prefab Library](../IDE_Tools/Prefab_Library.md) (except for those that have been duplicated into the project) as unused Prefab assets will always be removed during compilation.
...
- [Reddit](Game_Options/Reddit_Game_Options.md)
...
- [tvOS](Game_Options/tvOS.md)
  ...
  The various **Console Game Options** are not listed here due to the various legal restrictions maintained by the different companies. If you have a Console Licence, then you can get this information from the [GameMaker Wiki](https://github.com/YoYoGames/GameMaker-Bugs/wiki#console-setup-guides), but you must first contact YoYo Games with your console ID email and request permission to view the different sections.
