...
After logging in and starting a new project, GameMaker will take you to the initial **workspace** with some basic windows docked to the [IDE](#).

The workspace is simply an area where you can organise the different assets for your game while you are working. It has the following parts:

[Welcome Tab](#)

The first thing you see will be the **Welcome Tab** that explains the most fundamental parts of the IDE:

This tab will appear on every project until it is manually closed from the top (by clicking the x button next to "**Welcome Tab**"). This can be reopened from the [Help menu](../IDE_Navigation/Menus/The_Help_Menu.md).

The initial workspace is on a tab at the top of the screen (and you can rename it by double clicking the tab), but you can create further workspaces for the project by clicking the  to the side, giving you multiple possible workspaces for any single project. For example, maybe you are working on interactions between the player and several enemy objects, so you'd have the player on its own workspace and the enemy objects in another, and perhaps another workspace only to show the scripts that both use.
...
|  | Opens [The Start Page](The_Start_Page.md) in a new workspace. |
| --- | --- |
|  | Create a new project. |
|  | Open a previously created project. |
|  | Save the current project. Note that by default GameMaker will save your project automatically (depending on the [Preferences](../Setting_Up_And_Version_Information/IDE_Preferences.md) that you have set) and this button is simply so you can force a save at any time. |
|  | This will compile the current project and create an [executable package](../IDE_Navigation/Menus/The_Build_Menu.md#) (or packages) for the target platform. It will first open the file explorer so that you can select where you want to save the project and then compile the game. See the section on [Compiling](Compiling.md) for more information on setting up the compile targets. |
|  | This will create an executable, just like the option above, and then it will launch that executable on the target platform. |
|  | Clicking this will test\-run your current project along with the debug module. Use this to test and debug your game. You can find more about the debug module from the section on [The Debugger](../IDE_Tools/The_Debugger.md). |
