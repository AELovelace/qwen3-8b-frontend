...
Finally, you can flag the path as being *Absolute*or not. By default the instance will start following the path from its current position (the relative position), but if you flag the *Absolute* checkbox then it will start following the
 path exactly as you designed and placed it in the Path Editor (the absolute position). The image below illustrates this:

As you can see in the image above, the two instances are following the *same* path started
