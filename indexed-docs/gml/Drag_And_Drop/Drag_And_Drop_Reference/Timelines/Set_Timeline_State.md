...
This action can be used to start, stop, pause or resume a timeline. The timeline must first have been set for the instance using the action [Set Instance Timeline](Set_Instance_Timeline.md), and then you can use this action at any time to
 change its running state from the following options
...
The above action block code will check the value held in a global variable and
 if it is true then the timeline is paused, and if it is false then the timeline will be resumed.
