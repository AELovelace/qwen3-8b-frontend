...
This action will close the currently open Ini file. This action should be called the moment you are finished reading or writing to any open Ini file (Ini files are opened using the action [Open Ini File](Open_Ini_File.md)). If you do *not* use this action after you have used the [Write To Ini File](Write_To_Ini_File.md) action, then nothing will be written to disk as the file information is held in memory until this action is called, at which point it is taken from memory
