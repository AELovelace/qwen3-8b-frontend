...
With this action you can write a numeric value or a string to an Ini file. You must previously have opened the Ini file for reading/writing using the action [Open Ini File](Open_Ini_File.md), and then with this action you give the type of
...
As you can see, Ini files are split into sections \- where each section has its own header name \- and
 then each section is subsequently split into various key/value pairs (much like a [Map Data Structure](../Data_Structures/Create_Map.md)). Note that you can create Ini files yourself using any text processing software (like Notepad on
 Windows) and then include them as an [Included File](../../../Settings/Included_Files.md) to read from and write to in your game.
