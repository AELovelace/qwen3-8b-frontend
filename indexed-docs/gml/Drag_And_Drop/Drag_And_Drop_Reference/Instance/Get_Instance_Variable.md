...
- [Persistent](../../../GameMaker_Language/GML_Reference/Asset_Management/Instances/Instance_Variables/persistent.md) \- The persistence toggle variable (default is false unless set otherwise in [The Object Editor](../../../The_Asset_Editors/Objects.md), set to true to make persistent). This will make an instance "persist" from room to room, i.e.: the instance will not be cleaned up on room end but instead will be carried over to the next room. Note that when an instance is persisted it will *not* call its Create nor Destroy event, but will call the Clean Up, Room End, and Room Start events.
