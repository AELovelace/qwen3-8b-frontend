...
You can also use the GML function [array\_create](../GML_Reference/Variable_Functions/array_create.md) to initialise an array with a fixed size, and you can even create "empty" arrays with *no* values, for example:
...
The [Garbage Collector](../GML_Reference/Garbage_Collection/Garbage_Collection.md) will automatically clean up arrays that lose references, i.e. no active variables point to the array now. This means that setting a variable that holds an array to *any other value* will eventually make the garbage collector remove the array from memory (assuming it was the only reference to the array).
