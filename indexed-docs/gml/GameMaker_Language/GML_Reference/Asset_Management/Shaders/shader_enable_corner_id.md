...
It sets a global state for all shaders being used where, when enabled, the shader "steals" 2 bits from the input colour values; one from the lowest bit of the red colour value, and one from the lowest bit of the blue colour value. These values can then be recovered in the shader to work out which vertex you are dealing with (i.e. which corner).
  ...
  This will not work when using [vertex buffers and primitives](../../Drawing/Primitives/Primitives_And_Vertex_Formats.md) as the colour data for each vertex is handled by the user.
...
vec2 rem \= mod(in\_Colour.rb \* 255\., 2\.);
int corner\_id \= int(dot(vec2(1\., 2\.), rem));
...
| 1 | 1 | 3 | Bottom\-left |
  ...
  The corner IDs are numbered consecutively going clockwise, starting at 0 for the top\-left corner.
...
| --- | --- | --- |
| enable | [Boolean](../../../GML_Overview/Data_Types.md) | Enable (true) or disable (false) this function. |
