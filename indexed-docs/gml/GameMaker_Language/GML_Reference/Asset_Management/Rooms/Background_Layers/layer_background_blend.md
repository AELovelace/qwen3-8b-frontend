...
Please note that you should try to limit blending on the **HTML5** platform (unless using WebGL), as each
 blended sprite has to be cached separately and so having many blended sprites may adversely affect performance (you can also set the cache size using the function [sprite\_set\_cache\_size()](../../Sprites/Sprite_Manipulation/sprite_set_cache_size.md)).
...
| blend |  | The colour to blend with the background sprite (default is c\_white) |
