...
- The **tile map** is what you call the collection of tiles that are added to a layer, either in the Room Editor or through code, as a single element on that layer.
  ...
  In the Room Editor you are limited to a single tile map per layer, which is created when you add a [Tile Layer](../../../../../The_Asset_Editors/Room_Properties/Layer_Properties.md), but when working with them in code, you can have multiple tile maps assigned to a layer.
...
| --- | --- |
| tile\_rotate | Used to set/get the rotate bit of a tile data blob. |
| tile\_mirror | Used to set/get the mirror bit of a tile data blob. |
| tile\_flip | Used to set/get the flip bit of a tile data blob. |
| tile\_index\_mask | A special constant that is for "and"\-ing with the tile data blob to extract the 19 bits of the tile index. |
...
- [layer\_tilemap\_set\_colmask](layer_tilemap_set_colmask.md)
- [layer\_tilemap\_get\_colmask](layer_tilemap_get_colmask.md)
