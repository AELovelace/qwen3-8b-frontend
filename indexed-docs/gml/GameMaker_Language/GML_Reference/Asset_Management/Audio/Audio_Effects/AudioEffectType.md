...
This is one of the built\-in [Enums](../../../../GML_Overview/Variables/Constants.md#enumhead) that GameMaker provides. Pass a value from this Enum to [audio\_effect\_create](audio_effect_create.md) to determine the type of audio effect that you want to create.
...
This is a [bitcrusher](https://en.wikipedia.org/wiki/Bitcrusher) effect to create **distortion**. It does this in four steps (or *stages*), that take place in the following order: 
...
This is a [high\-pass filter](https://en.wikipedia.org/wiki/High-pass_filter) (shortened to *HPF*). A filter is used for filtering out certain frequencies.
...
This is a [low\-pass](https://en.wikipedia.org/wiki/Low-pass_filter) filter (shortened to *LPF*). A filter is used for filtering out certain frequencies.
...
This is a [reverb](https://en.wikipedia.org/wiki/Reverb_effect) effect. It combines the original signal with a reverberated version of it. High frequencies can be *dampened* (reduced).
...
This is a [tremolo](https://en.wikipedia.org/wiki/Tremolo_(electronic_effect)) effect. It makes use of an [LFO](AudioLFOType.md#LFO) (Low\-Frequency Oscillator) to modulate audio effect parameters at audio rates (as opposed to game\-frame rates).
