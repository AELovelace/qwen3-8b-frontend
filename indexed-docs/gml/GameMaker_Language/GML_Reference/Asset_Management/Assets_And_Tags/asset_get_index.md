...
This function gets the handle for an [Asset](../../../../The_Asset_Editors/The_Asset_Editors.md) from its name.

  If an [Asset](../../../../The_Asset_Editors/The_Asset_Editors.md) handle is passed, its index number is returned (similar to passing a handle into [real](../../Variable_Functions/real.md)). 

If the asset is not found, the function will return a value of \-1\. This handle can then be used in other functions as you would any other handle, like [sprite\_index](../Sprites/Sprite_Instance_Variables/sprite_index.md) or [path\_index](../Paths/Path_Variables/path_index.md), for example.
...
| --- | --- | --- |
| name | [String](../../../GML_Overview/Data_Types.md) or [Asset](../../../../The_Asset_Editors/The_Asset_Editors.md) | The name of the game asset to get the handle of (a string). |
...
The above code first marks all assets with an "include\_me" tag as "used" in a call to [gml\_pragma](../../OS_And_Compiler/gml_pragma.md) in a script asset. In an object's Create event, a random number from 0 to 7 is chosen for the current instance with [irandom](../../Maths_And_Numbers/Number_Functions/irandom.md) and asset\_get\_index is called to retrieve the NPC sprite with that suffix number. Finally, in the Draw event, the chosen sprite is drawn at the instance's position.  
