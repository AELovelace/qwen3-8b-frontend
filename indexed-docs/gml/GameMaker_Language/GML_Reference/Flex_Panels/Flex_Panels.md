...
Each node has styling properties, which define its position, size and behaviour within its container. You can specify these styling properties in the struct/JSON that you pass while creating a node, which are all listed here: [Styling Properties](Flex_Panels_Styling.md#h20).
...
After you have set up your layout, you must call [flexpanel\_calculate\_layout](Function_Reference/flexpanel_calculate_layout.md) with the dimensions where you intend to use the layout.
...
Open the section below to see the code used to generate both of the layouts shown above:
...
The first child is centred because of its parent's [Justify Content](Flex_Panels_Styling.md#h13) and [Align Items](Flex_Panels_Styling.md#h1) properties (which handle the main axis and cross axis alignment, respectively).

The second child has its [Position Type](Flex_Panels_Styling.md#h17) set to "absolute", which allows it to be positioned along the bottom\-right corner independently.
...
The "column" [Flex Direction](Flex_Panels_Styling.md#h8) of the parent allows its children to be laid out vertically (forming the main axis), and the "flex\-end" [Align Items](Flex_Panels_Styling.md#h1) property aligns the children to the end of its cross axis (i.e. to the right).

The [Flex Shrink](Flex_Panels_Styling.md#h10) property in the children ensures they shrink if they overflow the bounds of its parents.
...
You can use this to test your JSON data and edit layouts before using the data in your game code. This JSON code can also be used as a struct literal in your code.

## Containing Block

Some of the [Styling Properties](Flex_Panels_Styling.md#h20) depend on a node's Containing Block, which may not be the same as its direct parent. For example, a node with a positionType of relative will position itself within its direct parent, however an absolute node will position itself to its Containing Block.
