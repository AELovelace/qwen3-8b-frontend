...
This function clears the current state of the given mouse button. This means that checks for it being held down will not return true until the player releases the button and presses it again.
  ...
  This function only clears the state of the mouse button. The release will still be detected when the button is released on the mouse.
...
| --- | --- | --- |
| button | [Mouse Button Constant](mouse_check_button.md) | Which mouse button constant to check for. |
