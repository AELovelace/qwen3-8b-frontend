...
The function will take a keycode value as returned by the function [ord](../../Strings/ord.md) (only *capital* letters from A\-Z or numbers from 0\-9\), or any of the vk\_\* constants listed on the main [Keyboard Input](Keyboard_Input.md) page.
  ...
  This function is only available for the standard Windows target and the result is independent of which application has focus. On all other platforms it behaves the same as [keyboard\_check](keyboard_check.md).
...
| --- | --- | --- |
| key | [Virtual Key Constant (vk\_\*)](Keyboard_Input.md) | The key to check the down state of. |
