...
Also note that to use the [tilt functions](#func_ref_tilt) on GX.games you need to call [os\_request\_permission](../../OS_And_Compiler/os_request_permission.md)("DeviceMotion") to access orientation information. This only applies to certain browsers, however. More specifically, it applies to those browsers that have a checkmark in the [Browser compatibility table](https://developer.mozilla.org/en-US/docs/Web/API/DeviceMotionEvent#browser_compatibility) (in the requestPermission() static method row).
