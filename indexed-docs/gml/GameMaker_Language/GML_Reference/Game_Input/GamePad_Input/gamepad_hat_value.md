...
Note that these can be combined; e.g. a hat value of 3 would be checking up and right (the sum of 1 and 2\). You can also use the [bitwise operator](../../../../Additional_Information/Bitwise_Operators.md) \|to combine the values of various directions to get the same result, e.g. 0b0001 \| 0b0010. A value of 0 means no direction is set.
  ...
  Only values where *at most 2 bits* are set can be returned (e.g. a value of 7 will never be returned because it is impossible to press up (1\), right (2\) and down (4\) all at the same time).

  This function also returns a bitmask for gamepads that support analog input for hats.
...
| --- | --- | --- |
| device | [Real](../../../../../GameMaker_Language/GML_Overview/Data_Types.md) | Which gamepad device "slot" to check. |
| hatindex | [Real](../../../../../GameMaker_Language/GML_Overview/Data_Types.md) | Which gamepad hat (or hats) to check for (a value from 0 to [gamepad\_hat\_count](gamepad_hat_count.md)(device) \- 1) |

 

#### Returns:

[Real](../../../../../GameMaker_Language/GML_Overview/Data_Types.md) (hat value as a bitmask)
