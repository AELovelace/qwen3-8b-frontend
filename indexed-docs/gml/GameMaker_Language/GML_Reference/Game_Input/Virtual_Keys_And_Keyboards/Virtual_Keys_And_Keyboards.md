...
Note that these "virtual keys" are mapped to the GUI coordinates rather than room position and so the x/y values are absolute, based on the [GUI size](../../Cameras_And_Display/display_set_gui_size.md). This means that you don't need to worry about the use of views or the relative room coordinates, and can simply define the keys in the Create Event of an object (you only need to define a virtual key once per room), then draw your key sprites in the [Draw GUI Event](../../../../The_Asset_Editors/Object_Properties/Draw_Events.md).
  ...
  These functions are supported on the HTML5 and GX.games targets.

  Using virtual keys with the three\-finger screenshot gesture enabled can cause issues with the receiving of touch releases on some Android devices. A fix is in place that doesn't release all touches immediately, but only when a new touch is detected. It is, however, recommended that you recommend players to disable this gesture as this avoids the issue altogether.
...
- [virtual\_key\_delete](virtual_key_delete.md)
...
- [keyboard\_virtual\_set\_position](keyboard_virtual_set_position.md)
