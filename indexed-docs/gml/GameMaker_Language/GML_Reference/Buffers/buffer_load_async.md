...
This function loads a file that you have created previously using the [buffer\_save](buffer_save.md) function (or any of the other functions for saving buffers) into a buffer.

The "offset" defines the start position within the buffer for loading (in bytes), and the "size" is the size of the buffer area to be loaded from that offset onwards (also in bytes). You can supply a value of \-1 for the size argument to load the entire buffer. Note that the function will load from a "default" folder (unless you are using an [async group](buffer_async_group_begin.md), then the name of the group replaces "default"), which does *not* need to be included as part of the file path you provide. This folder will be created if it doesn't exist or when you save a file using [buffer\_save\_async](buffer_save_async.md).
...
| --- | --- | --- |
| buffer | [Buffer](buffer_create.md) | The buffer to load. |
| filename | [String](../../GML_Overview/Data_Types.md) | The name of the file to load. |
| offset | [Real](../../GML_Overview/Data_Types.md) | The offset within the buffer to load to (in bytes). |
| size | [Real](../../GML_Overview/Data_Types.md) | The size of the buffer area to load (in bytes). |
...
The above code loads the contents of the file "*Player\_Save.sav*" to the given buffer, storing the ID of the function call in the variable loadid. When the load is complete, the asynchronous [Save / Load](../../../The_Asset_Editors/Object_Properties/Async_Events/Save_Load.md) event will be triggered and you can parse the [async\_load](../../GML_Overview/Variables/Builtin_Global_Variables/async_load.md) map for the correct ID of the function, like this:
