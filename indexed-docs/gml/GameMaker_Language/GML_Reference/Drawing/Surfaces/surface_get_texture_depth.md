...
| --- | --- | --- |
| surface | [Surface](surface_create.md) | The surface of which to get the depth texture |
...
Finally, in the Draw GUI event the depth texture of the [application\_surface](application_surface.md) is retrieved with a call to surface\_get\_texture\_depth and stored in a temporary variable \_texture. The depth texture is then drawn in the top\-right corner of the window on top of the application surface. It is displayed on a textured primitive drawn using the [Primitive Functions](../Primitives/Primitives_And_Vertex_Formats.md#h), using a shader sh\_display\_depth that uses the fragment shader code and the LinearizeDepth() function provided on this page.
