...
Sprites and backgrounds are always drawn without repeating, so once you draw a sprite or background this value is reset to false. This default behaviour will not affect texture stage settings (i.e.: calling the function will, but the internal setting to false will not).

This setting will override any texture stage repeats set for shaders when using the function [gpu\_set\_texrepeat\_ext](gpu_set_texrepeat_ext.md).
