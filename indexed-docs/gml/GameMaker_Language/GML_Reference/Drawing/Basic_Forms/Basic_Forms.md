...
GameMaker has a whole collection of functions available for drawing different forms and shapes. These functions should only ever be used in [Draw Events](../../../../The_Asset_Editors/Object_Properties/Draw_Events.md) of an instance (you *can* use them elsewhere but you won't see anything!) and create purely graphic effects, which means that they do not generate collision events as those can only be done through the sprite or collision mask that has been assigned to an instance. All these functions are affected by the current draw colour, alpha and blend mode (see the section on [Colour](../Colour_And_Alpha/Colour_And_Alpha.md)).

  Lines, points and forms are always drawn at 1:1 with the **screen resolution**, regardless of the event they are drawn in.

  These functions will break the vertex batching and increase the number of texture swaps in your game, so having multiple calls to any of them in a single draw frame may lead to performance issues.

## Function Reference

  These functions are slow to call and only supplied as basic drawing tools.

- [draw\_arrow](draw_arrow.md)
- [draw\_circle](draw_circle.md)
- [draw\_circle\_colour](draw_circle_colour.md)
- [draw\_ellipse](draw_ellipse.md)
- [draw\_ellipse\_colour](draw_ellipse_colour.md)
- [draw\_line](draw_line.md)
- [draw\_line\_colour](draw_line_colour.md)
...
- [draw\_rectangle\_colour](draw_rectangle_colour.md)
- [draw\_roundrect](draw_roundrect.md)
- [draw\_roundrect\_colour](draw_roundrect_colour.md)
...
- [draw\_set\_circle\_precision](draw_set_circle_precision.md)
