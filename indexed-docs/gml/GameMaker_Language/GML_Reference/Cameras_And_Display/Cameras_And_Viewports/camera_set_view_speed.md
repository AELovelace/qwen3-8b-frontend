...
| --- | --- | --- |
| camera | [Camera ID](camera_create.md) | The unique camera ID value returned when you created the camera. |
| x\_speed | [Real](../../../GML_Overview/Data_Types.md) | The speed (number of pixels per game frame) that the view should move on the horizontal (x) axis to catch up to the target |
| y\_speed | [Real](../../../GML_Overview/Data_Types.md) | The speed (number of pixels per game frame) that the view should move on the vertical (y) axis to catch up to the target |
...
The above code will set the view camera speed for the camera assigned to viewport\[0].
