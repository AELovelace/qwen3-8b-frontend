...
GameMaker updates things based on the [game speed](../../General_Game_Control/game_get_speed.md), so that if you [set](../../General_Game_Control/game_set_speed.md) it to ,say, 30, GameMaker will run 30 steps in the course of a second. However, for the physics functions that may not be enough and you may want things to be updated at a slightly faster speed to increase stability or precision. To that end we use the function physics\_world\_update\_speed which sets the update speed for the physics system *independently* of the game speed. This means that you could have a game speed of 30, but set the physics to 60, effectively doubling the speed at which the physics system updates and performs its calculations compared to the speed at which the step are updated.
...
| --- | --- | --- |
| speed | [Real](../../../GML_Overview/Data_Types.md) | The number of times per second that the physics system updates |
