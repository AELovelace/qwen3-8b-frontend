...
You can also pass an array in the first argument, in which case the function will create a text field for each reference in the array.
  ...
  If the variable referenced stores a value of a different type than the type you provide to this function, any change you make to the text will change the referenced variable's type.

 
  On Android, editing a text field will bring up the on\-screen keyboard.
...
| label | [String](../../GML_Overview/Data_Types.md) | The label to show next to the text input |
| type | [String](../../GML_Overview/Data_Types.md) | The type to store the value as in the variable that's referenced. This is indicated by a single letter:    \- string: "s" or "t" (default)  \- integer: "i" or "d"  \- real: "f", "r" or "g" |
...
The above code first assigns some text to an instance variable description. It then creates a reference to this variable using [ref\_create](../Variable_Functions/ref_create.md). Next, it creates a text input control using dbg\_text\_input, the reference *links* the control to the variable.
...
In the object's Draw event a line of text is displayed using [draw\_text](../Drawing/Text/draw_text.md) for each of the variables. Each line shows a descriptive text and the type of value between parentheses, followed by the value itself. Any change made in the text inputs will show in the drawn text.
