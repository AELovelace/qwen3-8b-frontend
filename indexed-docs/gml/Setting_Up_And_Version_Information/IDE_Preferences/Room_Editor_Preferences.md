...
- **"Select from any layer" is switched off when Tile or Path layer auto\-selected**: This controls what happens when you click on the contents of a Tile or Path layer while "**Select from any layer**" is enabled. If enabled (default), "**Select from any layer**" will be disabled upon selecting such a layer and you will be able to edit that layer. If disabled, "**Select from any layer**" will stay enabled, but you will not be able to edit the selected Tile or Path layer until the mode is manually turned off.
...
- **Default Instances panel orientation**: Default orientation for the [Element List](../../The_Asset_Editors/Rooms.md#p) that appears when an Instance Layer or Asset Layer is selected.
- **Inspector selection colour**: The colour used for highlighting the item selected in the Inspector [Multi\-Select](../../IDE_Tools/The_Inspector.md#h) list.
...
- **Show node outlines**: Whether the outlines for Flex Panels should be shown or not. This is the same as the option displayed in the [Toolbox](../../The_Asset_Editors/Room_Properties/UI_Layers.md#h).
...
- **Canvas selection selects deepest element by default**: When this is disabled (default), clicking in the Room canvas selects the top\-level Flex Panel that is found under the mouse cursor and you are required to hold ALT to choose the deepest panel or element. Enabling this makes the ALT behaviour the default click behaviour so the deepest element under the mouse is always selected. See: [Selection Rules](../../The_Asset_Editors/Room_Properties/UI_Layers.md#h1)
- **Default asset anchor**: The default anchor value used for asset elements.
- **Changing Anchor adjusts position offset to keep same absolute position**: If enabled, changing the anchor for an asset element will modify its position values so that the final position of the asset remains the same. If disabled, changing the asset anchor will result in the asset moving to a different position as its position is relative to its anchor.
- **Confirm Dialogs**
